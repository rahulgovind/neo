...
class CommandBuilder:
    ...
    @staticmethod
    def parse_command_input(command_name: str, command_input: str, parameters: List['CommandParameter'], requires_data: bool = False) -> Tuple[Dict[str, Any], Optional[str]]:
        """
    ...
    @staticmethod
    def parse(command_name: str, command_line: str, parameters: List['CommandParameter']) -> Dict[str, Any]:
        """
    ...
    @staticmethod
    def format_manual(name: str, description: str, parameters: List['CommandParameter'], 
                    manual: str = "", epilog: str = "", extended: bool = False) -> str:
        """
        Format a command manual as a string, similar to a man page.
        
        Args:
            name: The name of the command
            description: Short description of the command
            parameters: List of CommandParameter objects defining the command's parameters
            manual: Extended manual text (shown only when extended=True)
            epilog: Additional description text
        ...
        if epilog:
            ...
            wrapped_desc = textwrap.fill(epilog, width=76, initial_indent="    ", 
                                        subsequent_indent="    ")
        ...
        if parameters:
            ...
            flags = [p for p in parameters if p.is_flag]
            if flags:
                for param in flags:
                    flag_str = []
                    if param.short_flag:
        ...
        # Extended manual section
        if extended and manual:
            lines.append("MANUAL")
            lines.append(manual)
            lines.append("")
