...
class Client:
            ...
            processed_messages.append(
                {
                    "role": message.role,
                    "content": [
                        {"type": "text", "text": message.text(), **additional_params}
                    ],
                }
            )
    ...
    def _parse_response(
        self, response, session_id: str, request_data: Dict[str, Any]
    ) -> Message:
        """
        ...
        message = Message(role="assistant", metadata=metadata)
        blocks = []
        ...
        current_block_end_marker = (
            False  # Whether the end marker is set for command calls
        )
        ...
        def flush_current_block():
            ...
            if current_block_type == "command":
                block = CommandCall(
                    content=buffer_content, end_marker_set=current_block_end_marker
                )
            else:  # text
