...
class Database:
    ...
    def get_session_messages(
        ...
        try:
            logger.debug(
                f"DB: Getting messages for session {session_id} with limit {limit}"
            )
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                if session_exists is None:
                    logger.warning(
                        f"DB: Attempted to get messages for non-existent session: {session_id}"
                    )
                    return []
            ...
            logger.debug(
                f"DB: Retrieved {len(messages)} messages for session {session_id}"
            )
            return messages
    ...
    def store_memory(self, session_id: str, data: Any) -> int:
        ...
        try:
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor.execute(
                    "SELECT id FROM memory WHERE session_id = ?", (session_id,)
                )
