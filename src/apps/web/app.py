...
class WebChat:
    ...
    def get_chat_history(self, session_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.debug(f"Session {session_id} exists in database, proceeding to get messages")
            ...
            try:
                ...
                with new_context(session_id=session_id, workspace=self.workspace) as ctx:
                    logger.debug(f"Context setup successful for session {session_id}, getting messages")
                    messages = self.db.get_session_messages(session_id)
                    logger.debug(f"Retrieved {len(messages)} messages for session {session_id} with context")
                    return messages
            except Exception as e:
                logger.error(f"Error setting up context for session {session_id}: {str(e)}")
                ...
                messages = self.db.get_session_messages(session_id)
                logger.debug(f"Retrieved {len(messages)} messages for session {session_id} without context (fallback)")
                return messages
        except Exception as e:
    ...
    def get_sessions(self) -> List[Dict[str, Any]]:
        ...
        with new_context(session_id=str(uuid.uuid4()), workspace=os.path.expanduser("~")) as ctx:
...
def _get_log_entries(session_id: str, logger_name: str) -> List[Dict[str, Any]]:
    ...
    # Match path construction logic in StructuredLogger._initialize
    log_file = Path(os.path.expanduser("~")) / ".neo" / f"session-{session_id}" / f"{logger_name}.yaml"
