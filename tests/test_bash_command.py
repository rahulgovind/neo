...
def test_bash_command(test_case):
    ...
    ctx = (
        Context.builder().session_id("test_session_id").workspace(temp_dir).initialize()
    )
    ...
    if test_case.name == "pwd_command":
        assert (
            temp_dir in result.result
        ), f"PWD output should contain the temp directory path for test case {test_case.name}"
    # Special case for cat_file (check content match)
    elif test_case.name == "cat_file":
        assert (
            "Content to be read with cat" in result.result
        ), f"Cat should show file content for test case {test_case.name}"
    # Check expected output for other cases if specified
    elif test_case.expected_output:
        assert (
            test_case.expected_output in result.result
...
def test_bash_failure_command(test_case):
    ...
    ), f"Error should not be None for test case {test_case.name}"

    # Verify the error message contains the expected substring
    assert (
        test_case.expected_error.lower() in result.error.lower()
