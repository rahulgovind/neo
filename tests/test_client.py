...
class TestClientE2E(unittest.TestCase):
    ...
    def test_cache_control(self):
        ...
        def generate_random_word_pairs(num_pairs=2000):
            ...
            logger.debug(f"Selected query word: {query_word} with expected ID: {expected_id}")
        ...
        for iteration in range(1, NUM_ITERATIONS+1):
            ...
            # Send the request
            logger.debug(f"Sending request {iteration} with cacheable message (length: {len(pairs_text)} characters)")
            response = client.process(
            ...
            assert relative_cache_discount_difference < 0.01, f"Cache discount error too large in iteration {iteration}"
